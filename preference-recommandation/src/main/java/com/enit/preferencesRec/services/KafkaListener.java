...
import com.enit.preferencesRec.config.EventService;
import com.enit.preferencesRec.config.MyStream;
import com.enit.preferencesRec.entity.*;
import com.enit.preferencesRec.events.*;
...
public class KafkaListener {
	...
	public void handleLoginRequest(@Payload String message) throws JsonProcessingException {
		...
		case CREATE_AD:
			...
			adsService.saveAd(new Ad(event2.getId(), event2.getCategory(), event2.getTitle(), event2.getDescription(),
					event2.getPrice(), event2.getAdvertiserPhoneNumber(), event2.getCountry(), event2.getState(),
					event2.getCity(), event2.getStatus(), event2.getAdImagesDirectory(), event2.getCondition(),
					event2.getModel(), event2.getBrand(), event2.getViews(), event2.getRate(),
					event2.getLocation().values().toArray(new Double[0])));
			System.out.println(event2.getType());
			break;
		case DELETE_AD:
			DeleteAdEvent event = objectMapper.readValue(message, DeleteAdEvent.class);
			// adsService.saveAd(new Ad(event.getCategory(),
			// event.getTitle(),event.getDescription(),event.getPrice(),
			// event.getAdvertiserPhoneNumber(),event.getCountry(),event.getState(),event.getCity(),event.getStatus(),event.getAdImagesDirectory(),event.getCondition(),event.getModel(),event.getBrand(),event.getViews(),event.getRate(),event.getLocation().values().toArray(new
			// Double[0])));
		...
		case UPDATE_AD:
			...
			adsService.saveAd(new Ad(adEvent.getId(), adEvent.getCategory(), adEvent.getTitle(),
					adEvent.getDescription(), adEvent.getPrice(), adEvent.getAdvertiserPhoneNumber(),
					adEvent.getCountry(), adEvent.getState(), adEvent.getCity(), adEvent.getStatus(),
					adEvent.getAdImagesDirectory(), adEvent.getCondition(), adEvent.getModel(), adEvent.getBrand(),
					adEvent.getViews(), adEvent.getRate(), adEvent.getLocation().values().toArray(new Double[0])));
	...
	public void handleUserRecommandation(@Payload String requestString) throws JsonProcessingException {
		...
		List<Ad> listAds1 = adsService.findByLocationAndPreferences(100, request.getPreferences(), request.getLar(),
				request.getLon());
		System.out.println("\n ***** There is " + listAds1.size() + "  recommandations for " + user_id + "\n");
//check for each ad in listAds1 if the adOwner is in the expiredOwners. If yes remove the ad from listAds1.

		// System.out.println(request);
		// System.out.println(user_id);
		if (listAds1.size() > 0) {// This is why we called this microservice:RandomRecommandation

			List<Recommandation> listRec = listAds1.stream().map(ads -> new Recommandation(ads))
					.collect(Collectors.toList());
			ListRecommandation rec = new ListRecommandation(listRec, user_id);
//           System.out.println("\n#### Recommandations' List to be sent is: "+rec);
